...
class WeekFragment : Fragment(R.layout.fragment_week) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = FragmentWeekViewAdapter()
        ...
        val events2 = database.getEventsInRange(start, end)
        val events3 = database.getEventsInRange(start, end)
        adapter.submitList(events)
        Handler().postDelayed({
            adapter.submitList(events2)
        }, 3000)
        Handler().postDelayed({
            adapter.submitList(events3)
        }, 7000)
    }
